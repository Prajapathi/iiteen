        ...
        if (doc.data() == null) {
          ...
          if (lvl2) {
            for (let i = 0; i < lvl2.length; i++) {
              if (lvl2[i].isAnswered === true) totalQAttempted++;
              if (flag == 0) {
                lastInd2 = i;
                flag = 1;
              }
          ...
          if (lvl3) {
            for (let i = 0; i < lvl3.length; i++) {
              if (lvl3[i].isAnswered === true) totalQAttempted++;
              if (flag == 0) {
                lastInd3 = i;
                flag = 1;
              }
    ...
    <div className="flip-card subject-card">
      <div className="flip-card-inner">
        <div className="flip-card-front">
          ...
          <div id="card-content">
            <div style={{ fontSize: "24px", color: "#448698" }}>
              {props.name}
            </div>
            <div style={{ width: "80%" }}>
              ...
              />
              <div style={{ color: "#448698", marginTop: "5px" }}>
                {" "}
                {totalAttemped}/75
              </div>
            </div>
        ...
        <div className="flip-card-back">
          <button onClick={() => selectLevel(1)}>
            <div style={{ fontWeight: "800" }}>Level &nbsp; &nbsp; 1</div>
          </button>
          <button onClick={() => selectLevel(2)}>
            <div style={{ fontWeight: "800" }}>Level &nbsp; &nbsp; 2</div>
          </button>
          <button onClick={() => selectLevel(3)}>
            <div style={{ fontWeight: "800" }}>Level &nbsp; &nbsp; 3</div>
          </button>
        </div>
      </div>
    </div>
