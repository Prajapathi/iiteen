...
export function Paper(props) {
  ...
  const markdistributionbyanswertype =
    mockpaperType == "ADVANCE"
      ? [[4, 0, 0], [2, 0, 0], [], [3, 1, 0], [4, 2, 0]]
      : [[], [4, 0, 0], [], [4, 1, 0], []];
  ...
  React.useEffect(() => {
    if (index == 0 || index == props.paper.noOfQuestions / 3 - 1)
      ...
      index == props.paper.noOfQuestions / 3 ||
      index == (2 * props.paper.noOfQuestions) / 3 - 1
    )
      setPalleteSub(2);
    else if (
      index == (2 * props.paper.noOfQuestions) / 3 ||
      index == props.paper.noOfQuestions - 1
    )
      setPalleteSub(3);
  }, [index]);
  ...
  const navigateQuestion = (ind) => {
    ...
    } else setIndex((2 * props.paper.noOfQuestions) / 3 + ind);
  };
  ...
  const submitPaperFinal = () => {
    ...
    const paperRef = db
      .collection("User")
      .doc(props.user.uid)
      .collection(paperTypeRoute)
      .doc(`${props.paper.sections ? "ADVANCE" : "MAINS"}`)
      .collection("PAPER")
      .doc(`PAPER${props.paper.number}`)
      ...
      .then((res) => {
            ...
            switch (Number(props.paper.questions[i].subject)) {
              case 1:
                ...
                if (!item.isAnsweredWrong) {
                  ...
                  totalCorrect++;
                  physicsMarks +=
                    markdistributionbyanswertype[
                      Number(props.paper.questions[i].answerType) - 1
                    ][0];
                  marks +=
                    markdistributionbyanswertype[
                      Number(props.paper.questions[i].answerType) - 1
              ...
              case 2:
                ...
                if (!item.isAnsweredWrong) {
                  ...
                  console.log(
                    Number(props.paper.questions[i].answerType),
                    markdistributionbyanswertype[
                      Number(props.paper.questions[i].answerType)
                    ]
                  );
        ...
        db.collection("User")
          ...
          .then((res) => {
            // window.alert("yo");
            history.push({
              pathname: "Analysis/" + `PAPER${props.paper.number}`,
            });
          })
          .catch((err) => {
  ...
  return !start ? (
    ...
    !showPaperInst ? (
      <>
        <GeneralInstruction
          setContinue={setShowPaperInst}
          mockpaperType={mockpaperType}
        />
        {/* <Timer duration={1}/> */}
      </>
    ) : (
      <PaperInstruction
        start={setStart}
        goToGeneralInst={() => setShowPaperInst(false)}
        details={props.paper}
        inst={props.paper.sections}
      />
    )
  ) : (
    ...
    <>
      <div
        className="timer-bar"
        style={{ justifyContent: showSummary ? "center" : "space-between" }}
      >
        <div>
      ...
      {!showSummary ? (
          ...
          >
            <div style={{ width: "80%" }}>
              <Question
                ...
                number={
                  questions && questions[index] ? questions[index].number : 0
                }
                ...
                <div id="ques-pallete-sub">
                  ...
                  <div
                    ...
                    onClick={() => {
                      setPalleteSub(2);
                    //   navigateQuestion(0)
                    setIndex(props.paper.noOfQuestions / 3 );
                    }}
                  >
                    Chemistry
                  </div>
                  <div
                    ...
                    onClick={() => {
                      setPalleteSub(3);
                    //   navigateQuestion(0)
                    setIndex((2 * props.paper.noOfQuestions) / 3 );
                    }}
                  >
                    Maths
                  </div>
                ...
                {palleteSub == 1
                      ...
                      <div
                        ...
                        style={{
                          background: props.answers[
                            props.paper.noOfQuestions / 3 + ind
                          ].isAnswered
                            ? props.answers[props.paper.noOfQuestions / 3 + ind]
                                .isBookmarked
                              ? "#ff9700"
                              : "#2AD586"
                            : props.answers[props.paper.noOfQuestions / 3 + ind]
                            ...
                            : "white",
                          border:
                            index == props.paper.noOfQuestions / 3 + ind
                              ? "1px solid black"
                              : null,
                        }}
                        onClick={() => navigateQuestion(ind)}
                  ...
                  : palleteArray.phy.map((text, ind) => (
                      <div
                        ...
                        style={{
                          background: props.answers[
                            (2 * props.paper.noOfQuestions) / 3 + ind
                          ].isAnswered
                            ? props.answers[
                                (2 * props.paper.noOfQuestions) / 3 + ind
                              ].isBookmarked
                            ...
                            : "white",
                          border:
                            index == (2 * props.paper.noOfQuestions) / 3 + ind
                              ? "1px solid black"
                              : null,
                        }}
                        onClick={() => navigateQuestion(ind)}
        ...
        <PaperSummary
          resume={setShowSummary}
          submit={submitPaperFinal}
          totalQ={props.paper.toBeAttempted}
          name={`PAPER${props.paper.number}`}
          answers={props.answers}
        />
      )}
    </>
